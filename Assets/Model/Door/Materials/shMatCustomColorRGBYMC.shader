Shader "Custom/shMatCustomColorRGBYMC" {
	Properties{
		_cRed("ColorRed", Color) = (1,0,0,1)
		_cGreen("ColorGreen", Color) = (0,1,0,1)
		_cBlue("ColorBlue", Color) = (0,0,1,1)
		_cYellow("ColorYellow", Color) = (1,1,0,1)
		_cMagenta("ColorMagenta", Color) = (1,0,1,1)
		_cCian("ColorCian", Color) = (0,1,1,1)
		...
		_gYellowMk("Glossiness Yellow Mask", Range(0,1)) = 0.5
		_gMagentaMk("Glossiness Magenta Mask", Range(0,1)) = 0.5
		_gCianMk("Glossiness Cian Mask", Range(0,1)) = 0.5
		...
		_mYellowMk("Metallic Yellow Mask", Range(0,1)) = 0.0
		_mMagentaMk("Metallic Magenta Mask", Range(0,1)) = 0.0
		_mCianMk("Metallic Cian Mask", Range(0,1)) = 0.0
		...
		_MaskTex02("Mask Texture (YCM)", 2D) = "black" {}
		_MaskEm("Mask Emission (RGB)", 2D) = "white" {}
...
#pragma target 3.0
		...
		sampler2D _MaskTex, _MaskTex02, _CompTex, _NormalTex, _MaskEm;

	struct Input {
		float2 uv_MaskTex;
	};

	half _gRedMk, _gGreenMk, _gBlueMk, _mRedMk, _mGreenMk, _mBlueMk, _gYellowMk, _gMagentaMk, _gCianMk, _mYellowMk, _mMagentaMk, _mCianMk;
	half _iAO, _iBorderDamaged, _iDirty, _gAO, _gBorderDamaged, _gDirty, _iNormalMap, _iLight;
	fixed4 _cRed, _cGreen, _cBlue, _cDirty, _cLight,_cBorderDamaged, _cYellow, _cMagenta, _cCian;

	void surf(Input IN, inout SurfaceOutputStandard o) {
		// Albedo comes from a texture tinted by color
		fixed4 _cMk = tex2D(_MaskTex, IN.uv_MaskTex);
		fixed4 _cMk02 = tex2D(_MaskTex02, IN.uv_MaskTex);
		...
		_finalColor = lerp(_finalColor, _cYellow, _cMk02.r);
		_finalColor = lerp(_finalColor, _cMagenta, _cMk02.g);
		_finalColor = lerp(_finalColor, _cCian, _cMk02.b);
		...
		_finalGlossiness = lerp(_finalGlossiness, _gYellowMk, _cMk02.r);
		_finalGlossiness = lerp(_finalGlossiness, _gMagentaMk, _cMk02.g);
		_finalGlossiness = lerp(_finalGlossiness, _gCianMk, _cMk02.b);
		...
		_finalMetallic = lerp(_finalMetallic, _mYellowMk, _cMk02.r);
		_finalMetallic = lerp(_finalMetallic, _mMagentaMk, _cMk02.g);
		_finalMetallic = lerp(_finalMetallic, _mCianMk, _cMk02.b);
